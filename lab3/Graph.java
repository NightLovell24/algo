...
public class Graph {
	...
	public void fillGraph() {
		...
		for (int i = 0; i < 200; i++) {
			...
			int rand = (int) ((Math.random() * 20) + 1); // [0;20) -> [1;21)
			List<Integer> listOfIds = new ArrayList<>();
			for (int j = 0; j < rand; j++) {

				int id = (int) (Math.random() * 200); // [0;200)
				while (id == i || listOfIds.contains(id)) {
					id = (int) (Math.random() * 200); // [0;200)
				}
				listOfIds.add(id);
				addEdge(i, id);
	...
	public void addEdge(int id1, int id2) {
		...
		if (vertex1 == null || vertex2 == null) {
			System.out.println("Нет таких узелков");
			return;
		} else {
...
//		System.out.println("ITERATIONS : " + iteration);
	...
	private void tryToColorVertex(Vertex vertex) {
		...
		for (Vertex vertexy : nearestVertices) {
			usedColorsVertices.add(vertexy.getColor()); // записываем все цвета вершин
...
//	public List<Graph> BeesAlgo(int iterations, Graph graph, int scoutBees , int foragerBees)
//     {
//       
